							...
							<div style="padding-top: 25px;">
								<?php
									...
									if (!isset($_GET['id']) || empty($_GET['id'])) {
										...
										$companyId = htmlspecialchars($_GET['id']);
										/* PRODUCTION BLOCK */
										$req = $bdd->query('SELECT * FROM COMPANY WHERE id=\'' . $companyId . '\'');
										$compData = $req->fetch();
										echo "<h1>" . $compData["NAME"] . " - " . country_code_to_country($compData["COUNTRY_CODE"]) . ": </h1>";
										
										$reqProdComp = $bdd->query('SELECT * FROM PRODUCTION_COMPANY WHERE COMPANY_ID=\'' . $companyId . '\'');
										
										while($dataProdComp = $reqProdComp->fetch()){
											$prod_id = $dataProdComp["PRODUCTION_ID"];
											$kind = $dataProdComp["KIND"];
											
											if ($prod_id != null) {
												$reqProd = $bdd->query('SELECT * FROM PRODUCTION WHERE ID=\'' . $prod_id . '\'');
												$prodData = $reqProd->fetch();
												
												echo "<h3><a href=\"production.php?id=" . $prod_id . "\"> " . $prodData["TITLLE"] ." </a></h3> <br /> ";
